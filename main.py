...
def format_number_with_word(number, word_forms):
    """
    word_forms: [именительный ед., родительный ед., родительный мн.]
    Пример: format_number_with_word(2, ["градус", "градуса", "градусов"]) -> "2 градуса"
    """
    try:
        num = int(number)
        if 11 <= num % 100 <= 19:
            return f"{num} {word_forms[2]}"
        last_digit = num % 10
        if last_digit == 1:
            return f"{num} {word_forms[0]}"
        if 2 <= last_digit <= 4:
            return f"{num} {word_forms[1]}"
        return f"{num} {word_forms[2]}"
    except:
        return f"{number} {word_forms[2]}"

def format_temperature(temp):
    """Форматирует температуру с правильным склонением"""
    return format_number_with_word(temp, ["градус", "градуса", "градусов"])

def format_wind_speed(speed):
    """Форматирует скорость ветра с правильным склонением"""
    return format_number_with_word(speed, ["метр в секунду", "метра в секунду", "метров в секунду"])

def format_humidity(humidity):
    """Форматирует влажность с правильным склонением"""
    return format_number_with_word(humidity, ["процент", "процента", "процентов"])
...
def get_weather_with_facts(city_name, day_offset=0, lang="ru"):
    ...
    temp_formatted = format_temperature(weather['temp'])
    wind_formatted = format_wind_speed(weather['wind'])
    humidity_formatted = format_humidity(weather['humidity'])
    
    if lang == "ru":
        response = f"{weather['emoji']} *{day_capitalized}* {temp_formatted}, {weather['condition'].lower()}, {wind_formatted}, {humidity_formatted}.\n\n"
...
def get_weather_for_voice_by_city(weather, fact, day_text, lang="ru"):
    ...
    if lang == "ru":
        return f"{day_text}, {temp_formatted}, {weather_desc}. {wind_formatted}. {humidity_formatted}. {fact}. Хотите узнать о погоде в другом городе? Спросите меня."
    else:
        return f"{day_text}, {temp_formatted}, {weather_desc}. {wind_formatted}. {humidity_formatted}. {fact}. Want to know the weather in another city? Ask me."
...
def get_weather(lat, lon, lang="ru"):
    try:
        ...
        temp_formatted = format_temperature(temp)
        wind_formatted = format_wind_speed(wind_speed)
        humidity_formatted = format_humidity(humidity)
        
        if lang == "en":
            weather_text = f"{emoji} {temp_formatted}, {weather_desc} {wind_formatted} {humidity_formatted}"
        else:
            weather_text = f"{emoji} {temp_formatted}, {weather_desc} {wind_formatted} {humidity_formatted}"
        return weather_text
    except:
...
def get_weather_for_voice(lat, lon, lang="ru"):
    """Голосовое сообщение о погоде с правильными склонениями"""
    try:
        url = f"https://wttr.in/{lat},{lon}?format=j1&lang={lang}&m"
        response = requests.get(url, timeout=10)
        response.encoding = 'utf-8'
        ...
        return f"сегодня {temp_formatted}, {weather_desc}. {wind_formatted}. {humidity_formatted}."
    except Exception as e:
...
def handle_text_message(chat_id, text):
    ...
    if any(greeting in text_lower for greeting in greetings):
        if lang == "ru":
            ...
            voice_text = "Привет-привет! Я DeVox, твой голосовой помощник в мире путешествий. Чем могу помочь? Расскажу о любом городе, покажу где вкусно поесть, подскажу погоду, посоветую что посмотреть, и даже помогу с билетами. Спрашивай что угодно о путешествиях!"
        else:
            response = "🐺 *Hello!* I'm DeVox — your voice assistant in the world of travel!\n\n✨ *How can I help?*\n\n• Tell you about any city\n• Show where to eat\n• Tell the weather\n• Suggest what to see\n• Even help with tickets!\n\n🌍 *Ask me anything about travel!*"
            voice_text = "Hello! I'm DeVox, your voice assistant in the world of travel. How can I help? Tell you about any city, show where to eat, tell the weather, suggest what to see, even help with tickets. Ask me anything about travel!"
        send_message(chat_id, response, get_pet_only_keyboard())
        text_to_voice_yandex(voice_text, chat_id, lang)
        return
    ...
    if city and is_weather:
        ...
        if weather and fact:
            ...
            voice_msg = answer.replace('*', '').replace('_', '').replace('[', '').replace(']', '').replace('(', '').replace(')', '')
            text_to_voice_yandex(voice_msg, chat_id, lang)
        return
...
if __name__ == "__main__":
    ...
    print("✅ Погода: эмодзи + правильные склонения (2 градуса, 5 метров в секунду, 64 процента)")
    print("✅ Фраза: 'Хотите узнать о других городах? Спросите меня.'")
    print("✅ 'Где я?' - полная озвучка с правильными склонениями")
    print("=" * 50)
