...
def remove_keyboard(chat_id):
    ...
    send_animation(chat_id, ANIMATIONS["green_check"])
...
def send_animation(chat_id, animation_id):
    """Отправляет анимацию через sendAnimation (более надёжный метод)"""
    url = f"{BASE_URL}/sendAnimation"
    payload = {"chat_id": chat_id, "animation": animation_id}
    try:
        response = requests.post(url, json=payload, timeout=30)
        if response.status_code == 200:
            print(f"✅ Анимация отправлена: {animation_id[:20]}...")
        else:
            print(f"❌ Ошибка анимации: {response.status_code}")
        return response
    except Exception as e:
...
def send_random_thinking(chat_id):
    send_animation(chat_id, ANIMATIONS["thinking"])
...
def handle_pet(chat_id):
    ...
    if level == 1:
        animation_id = ANIMATIONS["pet_level_1"]
        hint = "🐺 Ещё разочек?"
    elif level == 2:
        animation_id = ANIMATIONS["pet_level_2"]
        hint = "🐺✨ Почти финал!"
    else:
        animation_id = ANIMATIONS["pet_level_3"]
        hint = "🌟 Ты настоящий друг!"
    
    send_animation(chat_id, animation_id)
...
def handle_message(message):
    ...
    if text == "/start":
        send_animation(chat_id, ANIMATIONS["welcome"])
        time.sleep(0.5)
...
if __name__ == "__main__":
    ...
    print("🤖 DeVox запущен на Render с sendAnimation!")
